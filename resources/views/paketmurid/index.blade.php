                            ...
                            @if (count($getbuypaket)>0)
                                @foreach ($getbuypaket as $item)
                                    @if ($item->id_groups==$group->id)
                                        @if($item->status == 0)
                                            <span class="badge badge-warning">Tunggu Proses Approve</span>
                                            <a href="{{url('/tryout/beli-paket',$group->id)}}" class="btn btn-danger">Beli Paket</a>
                                            <a href="{{url('approve-paket',$group->id)}}" target="_blank" class="btn btn-success">Contact Admin</a>
                                        @else
